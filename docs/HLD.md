...
| `D-61` | `providers` | **`MAGICHOUR_MODEL` is pinned to `wan-2.2`, and the configured model is validated against the 10s shot length at startup.** | `D-03` fixes 10s beats. `wan-2.2` allows 3–10s and 15s, so 10s is reachable; **`sora-2` allows only 4, 8, 12, 24, 36, 48, 60 and cannot produce 10s at all**. A model swap that silently broke beat duration would break `[PRD §How it works 1]`, so it fails the deploy rather than every job. |
